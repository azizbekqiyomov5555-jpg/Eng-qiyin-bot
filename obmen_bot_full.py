...
# --------------------
# Sozlamalar (TOKEN va ADMIN_ID shu yerda; tavsiya: sozlashlarni env orqali uzgartiring)
# --------------------
# !!! XAVFSIZLIK: tokenni ochiq joyga qoʻymang. Bu yerda siz soʻraganingiz bois joylashtirildi.
BOT_TOKEN = "8252463123:AAHo7gUPcko6ueGRjQ15Efqcsd13FFp7HIY"
ADMIN_ID = 7973934849
...
# single global bot will be created in main()
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        f"Assalomu alaykum, {user['name']}! 👋\nXush kelibsiz botimizga. Pastdagi tugma orqali crypto valuta sotib olishingiz va sotishingiz mumkin.",
...
async def confirm_handler_buy(message: types.Message, state: FSMContext, bot: Bot = None):
    if message.text == "⏹️ Bekor qilish":
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=main_menu_kb(message.from_user.id))
        return
    ...
    # use global bot instance (injected by Dispatcher); fallback to new Bot if not provided
    try:
        b = Bot.get_current()  # aiogram will set current Bot
    except Exception:
...
async def admin_order_cb(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "confirm":
        ...
        try:
            ...
            b = Bot(token=BOT_TOKEN)
        try:
            await b.send_message(order["user_id"], f"Sizning buyurtmangiz tasdiqlandi ✅")
        except Exception:
        ...
        try:
            ...
            b = Bot(token=BOT_TOKEN)
        try:
            await b.send_message(order["user_id"], f"Sizning buyurtmangiz bekor qilindi ❌")
        except Exception:
            logger.exception("Foydalanuvchiga reject xabari yuborishda xato:")
        await callback.answer("Bekor qilindi")

# --------------------
# Admin panel (start & main menu)
# --------------------
...
async def send_broadcast(message: types.Message, state: FSMContext):
    ...
    for uid in list(users.keys()):
        try:
            ...
            b = Bot(token=BOT_TOKEN)
        try:
            await b.send_message(int(uid), text)
            count += 1
        except Exception:
            failed += 1
            continue
...
# --------------------
# Admin: add / edit / delete currency handlers
# --------------------
...
# Add currency sequence already implemented earlier (handlers)
...
async def edit_currency_rate_choose(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"{message.text} uchun yangi buy kursini kiriting:", reply_markup=back_kb())
...
async def edit_currency_rate_set(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        return
    data = await state.get_data()
    # set buy_rate; ask sell next
    currencies[data["rate_name"]]["buy_rate"] = buy_rate
    await state.update_data(temp_buy_rate=buy_rate)
    await message.answer(f"Endi {data['rate_name']} uchun sell kursini kiriting:", reply_markup=back_kb())
    await AdminFSM.edit_rate_choose.set()  # reuse a state to capture next input
    # NOTE: for simplicity we reuse a flow; alternative: create another state to set sell_rate
...
async def main():
    if not BOT_TOKEN:
        logger.error("BOT_TOKEN is not set. Set it before running.")
        return

    # create global bot and dispatcher
    bot = Bot(token=BOT_TOKEN)
    dp = Dispatcher(storage=storage)
    dp.include_router(router)

    # set current bot (aiogram context)
    Bot.set_current(bot)
